...
st.markdown("""
    <style>
        ...
        .stSelectbox>div>div>div {color: #333333;}
    </style>
...
# Inputs del usuario en columnas
...
with col3:
    region_favorita = st.selectbox("Región vinícola favorita", ["Rías Baixas", "Rioja", "Valdeorras", "Cataluña", "Ribera del Duero"])

st.markdown("---")
st.header("🍽️ Elige tu plato")
...
if st.button("🍽️ Recomendar vino"):
    ...
    if "mensaje" in vino_info:
        ...
        with st.container():
            st.markdown("### 🥂 Recomendación estándar")
            st.markdown(f"**{vino_info['vino']}**")
            st.write(f"- **Uva:** {vino_info['uva']}\n- **Cosecha:** {vino_info['cosecha']}\n- **Región:** {vino_info['region']}")
            st.write(f"- **Precio por copa:** {vino_info['copa']}€\n- **Precio por botella:** {vino_info['botella']}€")
            st.info(vino_info["explicacion"])

        if "vino_premium" in vino_info:
            st.markdown("---")
            st.markdown("### 🍾 Recomendación Premium")
            st.markdown(f"**{vino_info['vino_premium']['vino']}**")
            st.write(f"- **Uva:** {vino_info['vino_premium']['uva']}\n- **Cosecha:** {vino_info['vino_premium']['cosecha']}\n- **Región:** {vino_info['vino_premium']['region']}")
            st.write(f"- **Precio por copa:** {vino_info['vino_premium']['copa']}€\n- **Precio por botella:** {vino_info['vino_premium']['botella']}€")
